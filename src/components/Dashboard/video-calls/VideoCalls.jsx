...
const VideoCalls = () => {
    ...
    const fetchSessions = async()=>{
      ...
      if(response){setsessions(response.sessions.events)
        setfinished(response.finished.events)
      }
        setloading(false)
    }
  fetchSessions()
  ...
  return (
    <div className='flex flex-col gap-y-5 mt-14 items-center justify-center w-full'>
      <div className='text-3xl font-bold mb-5'><u>Upcoming Sessions</u></div>
  ...
  <tbody>
      ...
      <div className='text-3xl font-bold mb-5'><u>Finished Sessions</u></div>
      {loading ? (<div className='h-[400px] flex items-center justify-center'><div className='spinner  '></div></div>): <table className='w-full'>
      <thead>
    <tr className="bg-gray-100">
      <th className="px-4 py-2 text-left text-gray-600 font-bold">Title</th>
  ...
  <tbody>
      {
        finished?.map((item)=>{
          ...
          return <tr key={item._id} className='border-b hover:bg-gray-50 transition duration-300 ease-in-out '>
           ...
           <td className="px-4 py-2">{user?.accountType === "Mentee" ? item.mentor.firstName+" "+item.mentor.lastName : item.mentee.firstName+" "+item.mentee.lastName }</td>
           <td className="px-4 py-2">{start.toLocaleDateString()+" "+start.toLocaleTimeString() + "-" + end.toLocaleTimeString()}</td>
           <td className="px-4 py-2 text-center"><button className={`${item.status === "Scheduled" ? "bg-yellow-300 hover:bg-yellow-400 " : item.status === "OnGoing" ? "bg-blue-600 hover:bg-blue-700 text-white " : item.status === "Completed" ? "bg-green-500 hover:bg-green-600 text-white" : "bg-red-500 hover:bg-red-600 text-white "} text-black font-semibold py-1 px-3 rounded-lg`} onClick={()=>{if(item.status === "OnGoing")handleJoin(item.roomId)}}>{item.status === "Scheduled" ? "Scheduled" : item.status === "OnGoing" ? "Join" : item.status === "Completed" ? "Completed" : "Cancelled"}</button></td>
          </tr>
        })
      }
      </tbody>
